...
| K8 | Reserve a bounded 4,096 characters for the note and price conversation with a linear item-plus-join metric; total projected payload stays under the ceiling | The reserve covers the note join and the metric charges every same-role `\n\n` separator before pre-merge survivors are assembled | Treat metadata or assembly separators as free |
